...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <nav class="flex text-sm text-slate-500 mb-2">
            ...
            <span class="text-slate-800 font-medium">Edit Client</span>
        </nav>
        <h2 class="text-2xl font-bold text-slate-800">Edit Client: <?= $client['company_name']; ?></h2>
    </div>

    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <form action="<?= base_url('clients/'.$client['id']); ?>" method="POST" class="p-8 space-y-6">
            <?= csrf_field(); ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-1">
                    ...
                    <div class="relative">
                        <select id="province_select" name="province_id" required
                            data-selected="<?= $client['province_id']; ?>"
                        ...
                        <input type="hidden" id="province_name" name="province_name" value="<?= $client['province_name']; ?>">
                ...
                <div class="space-y-1">
                    ...
                    <div class="relative">
                        ...
                        <input type="hidden" id="city_name" name="city_name" value="<?= $client['city_name']; ?>">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address_detail" rows="3" required
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all"><?= old('address_detail', $client['address_detail']); ?></textarea>
                </div>
            </div>

            <div class="pt-6 border-t border-slate-100 flex items-center justify-end space-x-3">
                <a href="<?= base_url('clients/'.$client['id']); ?>" class="px-6 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-100 rounded-xl transition-all">Cancel</a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchProvinces() {
        ...
        try {
            ...
            data.forEach(prov => {
                ...
                if (prov.id === selectedProvince) option.selected = true;
                provinceSelect.appendChild(option);
            });

            if (selectedProvince) {
                fetchCities(selectedProvince, selectedCity);
            }
        } catch (error) {
    ...
    // Fetch Cities
    async function fetchCities(provinceId, initialCityId = null) {
